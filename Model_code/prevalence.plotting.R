...
# list of columns in a given compartment in the syphSim output data.
index_to_pop_names <- function(index) {
  colnames(popmatrix)[which(popmatrix[index, ] == 1)]
}

pop_names_to_index <- function(names) {
  which(apply(popmatrix, 1, function(x) all(x[names] == 1)))
}


popnames <- list(
  i = c("Black", "Hispanic", "Other", "MSM/HIV-", "MSM/HIV+"),
  j = c("Male", "Female"),
...
#' these letters and the characteristics, see the popnames object.
#'
...
select_populations <- function(ijkl_subset = c('i', 'j', 'k', 'l')) {
  selected_populations <- lapply(ijkl_subset, function(x) popnames[[x]])
  # selected_pop <- list()
...
#'
#' We start with sol, the output of syphSim. sol is a dataframe which contains the
...
#'
#' @param sol The output of the C++ syphSim function
...
#' 2011:2016  for the Louisana contact tracing period,
#' 2005:2016  for the Massachusetts contact tracing period,
#' 2012:2016  for the calibration period, or
tidy_prevalence_by_demographic <- function(
  ...
  # 1911 == start.year - cal.start
  # -1 because of cpp's 0-indexing and R's 1-indexing ->
  # If we wanted to select 1911, the 0th year, we would
  ...
  if (which_infections == "y") {
    #y.ind <- y.index[1:32] # use 1:32 to exclude Female MSM
    #infections <- sol[, 1+y.ind]
    infections <- sol[, 1:32]
  } else if (which_infections == "z") {
    #z.ind <- z.index[1:32] # use 1:32 to exclude Female MSM
    #infections <- sol[, 1+z.ind]
    infections <- sol[, 1:32]
  } else if (which_infections == "all") {
    #y.ind <- y.index[1:32]
    #z.ind <- z.index[1:32]
    #infections <- sol[, 1+y.ind]
    #infections <- infections + sol[, 1+z.ind]
    infections <- sol[, 1:32]
  } else {
  ...
  subpop_infections <-
    lapply(selected_indices, function(subpop_col_indices) {
      ...
      female_msm <- which(subpop_col_indices %in% 32:40)
      if (length(female_msm)!=0) {
...
#'  an MCMC calibration of the syphilis model.
#'
...
tidy_prevalence_dfs_for_many_simulations <-
  function(trace_sample,
    ...
    # We'll use each theta from trace_sample to run the syphSim deterministic compartmental model
    sol_list <- list()
...
plot_faceted_prevalence <-
  function(prevalence_dfs,
    ...
    g <-
      ...
      ggtitle(
        paste0(
          "Breakdown of Calibrated Syphilis ",
          which_infections,
