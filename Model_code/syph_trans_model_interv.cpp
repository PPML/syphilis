...
List syphSim(
  ...
  NumericMatrix screen_r = as<NumericMatrix>(x["screen.r"]); //screening rate in prior infecteds matrix
  NumericVector alpha(dim); //screening rate (updated annually)
  NumericVector alpha_r(dim); //screening rate in prior infecteds (updated annually)
  ...
  for(int y=0; y<nYrs; y++){
    for(int w=0; w<52; w++){
      ...
      for(int n=0; n<dim; n++) {   
        ...
        Pop[n+dim*6] += (p_trt1[n]*(I1[n]+IR1[n]) + p_trt2[n]*(I2[n] + IR2[n]) + alpha[n]*(I1[n]+I2[n]) + alpha_r[n]*(IR1[n]+IR2[n])   + abx*(I1[n]+I2[n]+IR1[n]+IR2[n]) - lambda1*T1[n] + agingT1[n])*dt ; //dT1/dt
        Pop[n+dim*7] += (p_trt3[n]*(L1[n] + LR1[n]) + alpha[n]*L1[n] + alpha_r[n]*LR1[n] + abx*(L1[n]+LR1[n]) - lambda2*T2[n] + agingT2[n])*dt; //dT2/dt
        Pop[n+dim*8] += (p_trt4[n]*(L2[n] + LR2[n]) + alpha[n]*L2[n] + alpha_r[n]*LR2[n] + abx*(L2[n]+LR2[n]) - lambda3*T3[n] + agingT3[n])*dt ; //dT3/dt
        ...
        Pop[n+dim*11] += (delta*ER[n] - gamma1*IR1[n] - p_trt1[n]*IR1[n] -alpha_r[n]*IR1[n] - abx*IR1[n]  + agingIR1[n])*dt; //dIR1/dt
        Pop[n+dim*12] += (gamma1*IR1[n] - gamma2*IR2[n] - p_trt2[n]*IR2[n] -alpha_r[n]*IR2[n] - abx*IR2[n]  + agingIR2[n]) * dt; //dIR2/dt
        Pop[n+dim*13] += (gamma2*IR2[n] - gamma3*LR1[n] - p_trt3[n]*LR1[n] -alpha_r[n]*LR1[n] - abx*LR1[n] + agingLR1[n]) * dt; //dLR1/dt
        Pop[n+dim*14] += (gamma3*LR1[n] - p_trt4[n]*LR2[n] -alpha_r[n]*LR2[n] - abx*LR2[n]  + agingLR2[n]) * dt; //dLR2/dt
        Pop[n+dim*15] += (lambda[n]*(S[n]+SR[n])) * dt; //dInc/dt
        Pop[n+dim*16] += (lambda[n]*SR[n]) * dt; //dIncr/dt
        Pop[n+dim*17] += (rep_s[n]*p_trt1[n]*(I1[n]+IR1[n]) + rep*alpha[n]*I1[n] + rep*alpha_r[n]*IR1[n] ) * dt; //dD1/dt
        Pop[n+dim*18] += (rep_s[n]*p_trt2[n]*(I2[n]+IR2[n]) + rep*alpha[n]*I2[n] + rep*alpha_r[n]*IR2[n] ) * dt; //dD2/dt
        Pop[n+dim*19] += (rep_s[n]*p_trt3[n]*(L1[n]+LR1[n]) + rep*alpha[n]*L1[n] + rep*alpha_r[n]*LR1[n] ) * dt; //dD3/dt
        Pop[n+dim*20] += (agingNSA[n] + birthsNSA[n]) * dt; //dNSA/dt
        Pop[n+dim*21] += (rep_s[n]*p_trt4[n]*(L2[n]+LR2[n]) + rep*alpha[n]*L2[n] + rep*alpha_r[n]*LR2[n] ) * dt; //dD4/dt
        Pop[n+dim*22] += (rep_s[n]*p_trt1[n]*IR1[n] + rep*alpha_r[n]* IR1[n] + rep_s[n]*p_trt2[n]*IR2[n] + rep*alpha_r[n]*IR2[n] + rep_s[n]*p_trt3[n]*LR1[n] + rep*alpha_r[n]* LR1[n] ) * dt; //dDR/dt
      }
      }
