  ...
  for(int y=0; y<nYrs; y++){
    ...
    for(int w=0; w<52; w++){
      ...
      for(int n=0; n<dim; n++) {   
        ...
        Pop[n+dim*6] += (p_trt1[n]*(I1[n]) + p_trt2[n]*(I2[n]) + alpha[n]*(I1[n]+I2[n]) + abx*(I1[n]+I2[n]) - lambda1*T1[n] + agingT1[n])*dt ; //dT1/dt
        Pop[n+dim*7] += (p_trt3[n]*(L1[n]) + alpha[n]*(L1[n]) + abx*(L1[n]) - lambda2*T2[n] + agingT2[n])*dt; //dT2/dt
        Pop[n+dim*8] += (p_trt4[n]*(L2[n]) + alpha[n]*(L2[n]) + abx*(L2[n]) - lambda3*T3[n] + agingT3[n])*dt ; //dT3/dt
