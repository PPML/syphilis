...
###############################################################################################
### this version includes different screening interventions ###

prediction.epi <- function(theta, interv) { 
  interv=interv #specify the intervention
  ...
  screen.in <- unname(t(apply(t(apply(screen.trend, 1, function(x) x*rr.screen.s)), 1, FUN=screen.fun, rr.screen=rr.screen))) #calculate actual screening rates for each group by multiplying base rates by rr
  screen.in[screen.in<0.0001]<-0  # make sure that screening rate isn't <0
  
  #different screening interventions plus default
  # if(interv==0) {
  #   screen <- screen.in
  #   screen.r <- screen
  # }
  if (interv==1) {  #screening continues at 2016 levels
    screen  <- rbind(screen.in[rep(1, times=(cal.start-11)),], screen.in, screen.in[rep((nrow(screen.in)), times=interv.period+3),])
    screen.r <- screen
  }
  if (interv==2) {  #screen entire population annually
    max.screen <- rep(1,40)
    screen <- rbind(screen.in[rep(1, times=(cal.start-11)),], screen.in, matrix(rep(max.screen,each=(interv.period+3)),ncol=40))
    screen.r <- screen
    #browser()
  }
  if (interv==3) {  #screen MSM according to guidelines, screen rest of population at 2016 levels
    max.screen <- screen.in[nrow(screen.in),]  #screening in 2016
    max.screen[13:20] <- rep(c(1,4),4) #screen MSM low activity every year, high activity every 3 months (regardless of HIV status)
    screen <- rbind(screen.in[rep(1, times=(cal.start-11)),], screen.in, matrix(rep(max.screen,each=(interv.period+3)),ncol=40))
    screen.r <- screen
  }
  if (interv==4) {  #screen MSM according to guidelines, screen rest of population annually
    max.screen <- c(rep(1,12),rep(c(1,4),4),rep(c(1,1,1,1),5))
    screen <-rbind(screen.in[rep(1, times=(cal.start-11)),], screen.in, matrix(rep(max.screen,each=(interv.period+3)),ncol=40))
    screen.r <- screen
  }
  if (interv==5) {  #screen prior infecteds every 3 months, screen rest of population at 2016 levels
    screen  <- rbind(screen.in[rep(1, times=(cal.start-11)),], screen.in, screen.in[rep((nrow(screen.in)), times=interv.period+3),])
    max.screen.r <- rep(4,40)
    screen.r <- rbind(screen.in[rep(1, times=(cal.start-11)),], screen.in, matrix(rep(max.screen.r,each=(interv.period+3)),ncol=40))
  }
  if (interv==6) {  #screen prior infecteds every 3 months, screen rest of population annually
    max.screen <- rep(1,40)
    screen  <- rbind(screen.in[rep(1, times=(cal.start-11)),], screen.in,  matrix(rep(max.screen,each=(interv.period+3)),ncol=40))
    max.screen.r <- rep(4,40)
    screen.r <- rbind(screen.in[rep(1, times=(cal.start-11)),], screen.in, matrix(rep(max.screen.r,each=(interv.period+3)),ncol=40))
  ...
  rep <-rbind(rep.trend[rep(1, times=(cal.start-11)),], rep.trend, rep.trend[rep((nrow(rep.trend)), times=20),])  #expand reporting prob to cover entire model run time, assume reporting pre-calibration period = reporting at start of cal; time trend starts 10 years prior to cal start
  ...
  behav <-c(rep(1, times=(cal.start-11)), behav.trend, behav.trend[rep(length(behav.trend), times=20)]) #expand to cover entire model run time, assume behav pre-calibration period = value at start of calibration; trend starts 10 years prior to cal start
  ...
  params <-list(b=b,delta=delta, gamma=gamma,p.trt.1=p.trt.1, p.trt.2=p.trt.2, p.trt.3=p.trt.3,p.trt.4=p.trt.4, rep=rep,rep.symp=rep.symp, screen=screen,screen.r=screen.r, behav=behav,dur.imm=dur.imm) #parameters used by transmission model
  ...
  out.cpp<-syphSim(params,
                   ...
                   aging.nsa)
  sol<-out.cpp$out
  prev<-model.prev(sol, interv)  #calculate desired outputs for calibration, function stored in model.out.fun.R
