...
model.prev <- function(sol) {  #sol is the output from the transmission model, a matrix of model compartment sizes at each time step
  ...
  inf.total <- inf.ps+inf.latent #all early infections
  ...
  #diag.early.s.a.adj <-diag.early.s.a[,1:6] + cbind(diag.early.s.a[,7:8]*(p.s.1+0.06), diag.early.s.a[,7:8]*(p.s.2-0.06), diag.early.s.a[,7:8]*p.s.3) + cbind(diag.early.s.a[,9:10]*(p.s.1+0.03), diag.early.s.a[,9:10]*(p.s.2-0.03), diag.early.s.a[,9:10]*p.s.3)#redistribute MSM cases among other male subpops
...
#calculate distribution of early syphilis cases by stage at diagnosis
 ...
 #proportion of early cases diagnosed with early latent syphilis 
  ...
  list(early.inf.rate.m= diag.age.sex.fit.m, early.inf.rate.f.y= diag.age.sex.fit.f.y, early.inf.rate.f.o = diag.age.sex.fit.f.o, diag.rr = fit.diag.rr, p.diag.msm = p.diag.msm, n.inc=n.inc, p.diag.hiv=p.diag.hiv, p.diag.sec=p.diag.sec, p.diag.el=p.diag.el, diag.late=diag.age.sex.fit.ll, p.diag.early=p.diag.early, 
       p.prior.inc = p.prior.inc, p.prior.diag=p.prior.diag, prev.total=prev.total, prev.tot.sex=prev.tot.sex)
}
