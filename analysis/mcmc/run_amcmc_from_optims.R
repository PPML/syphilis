...
array_id <- args[[1]]
output_dir <- here("inst/mcmc/10-13-19/")
input_dir <- here("inst/optims/10-12-19/")
...
top_optim_trace_path_la <- paste0(input_dir, "la_top5_best_pars.rds")
optim_trace_la <- readRDS(top_optim_trace_path_la)
theta_la <- unlist(optim_trace_la[ (as.numeric(array_id) %% 5)+1, ])

top_optim_trace_path_ma <- paste0(input_dir, "/ma_top5_best_pars.rds")
