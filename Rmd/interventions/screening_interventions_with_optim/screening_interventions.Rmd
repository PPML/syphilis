...
for (state in c('LA', 'MA')) { 
	...
	df <- filter(df, year >= 2015 & year <= 2026) 
...
```{r plot data, fig.width=9, fig.height=4, echo=FALSE, dpi = 300}
for (state in c('LA', 'MA')) {
	df <- get(paste0(tolower(state), '_df'))
...
```{r define helper fn 2, echo=FALSE, fig.width=9, fig.height=4, dpi=300}
