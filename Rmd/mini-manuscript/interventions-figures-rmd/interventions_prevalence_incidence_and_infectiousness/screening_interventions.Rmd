...
simulate_annual_and_basecase <- function(theta) {
	...
	for (intervention in c('basecase', 'msm_annual_hr_msm_quarterly', 'prior_diagnosis_quarterly')) {
...
df$intervention %<>% factor(
	...
	labels = c('Base Case', 'Annual', 'MSM Annual', 'MSM Annual, High Activity MSM Quarterly', 'Prior Diagnosis (Past 2 Years) Quarterly'))
...
plt <- ggplot(
   ...
   scale_x_continuous(breaks = seq(2017, 2027, by = 2)) + 
	 ggtitle("Screening Intervention Outcomes - Prevalence and Incidence") + 
	 theme_bw() + 
...
ggsave("prevalence_and_incidence.png", width = 10)
...
ggplot(
   ...
   scale_x_continuous(breaks = seq(2017, 2027, by=2)) + 
	 ...
	 facet_wrap(state~variable, scales='free_y') + 
	 theme_bw() + 
   ggtitle("Intervention Scenario Outcomes – Prevalence of Infectious and Non-Infectious Syphilis") + 
	 guides(linetype = guide_legend(title='Intervention Scenario Screening Rates'),
					color = guide_legend(title='Intervention Scenario Screening Rates')) +
	 ylab("Infections per 100,000") + 
	 xlab("Year") + 
...
ggsave("infectious_and_noninfectious.png", width = 10)
```
