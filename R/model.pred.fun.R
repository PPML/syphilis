...
#' Run a Simulation with Parameters theta and the DiffEq Model in syph_trans_model.cpp
#' 
#' This function uses three steps: 
#' 1. constructSimulationEnvironment: This produces an environment object containing the 
#'    necessary arguments for the Rcpp function syphSim defined in syph_trans_model.cpp
#' 2. runSimulation: Using the objects in the environment `e`, run syphSim.
#' 3. summarizeSimulation: Run model.prev to extract summary features from the simulation.
#' 
#' This approach is preferred to a less modular (i.e. combining all the above
#' steps into one function) approach because in between steps 1 and 2 the simulation 
#' environment can be edited (for example to define intervention scenarios, to change or define 
#' debugging arguments, etc.), and if alternative summary measures are desired step 3 can be 
#' changed to any other function that analyzes the output from syphSim. 
#' 
#' @param theta A named numeric vector defining each of the model parameters.
#' 
...
#' Transform theta into an Environment containing Arguments Necessary for Simulation
#' 
#' 
constructSimulationEnvironment <- function(theta) {
  e <- new.env()
	with(e, {
...
prediction.epi.orig <- function(theta) { # fit to infection data
  ### return log/logit transformed parameters back to untransformed values and put in required form for use in transmission model ###
  epsilon <- ilogit(c(theta["logit.epsilon.1"], theta["logit.epsilon.2"], theta["logit.epsilon.3"], theta["logit.epsilon.4"], theta["logit.epsilon.5"])) #mixing between high and low activity groups (black, other, Hispanic, MSM HIV-, MSM HIV+)
  c.min.param <- exp(c(theta["log.c.min.m.1"], theta["log.c.min.m.2"], theta["log.c.min.f.1"], theta["log.c.min.f.2"], theta["log.c.min.msm.1"], theta["log.c.min.msm.2"]))
  c.min <- c.min.fun(c.min.param) #produces a vector of minimum partner change rates 
  ...
  screen[102:105,] <- replacement
	params <-list(b=b,delta=delta, gamma=gamma,p.trt.1=p.trt.1, p.trt.2=p.trt.2,
								...
								screen=screen, behav=behav,dur.imm=dur.imm,
								ct.data.years=ct.data.years, p.ct.primsec=p.ct.primsec, p.ct.el
								= p.ct.el, p.s.1=p.s.1, p.s.2 = p.s.2, p.s.3 = p.s.3, d=d,
								fileName="tintoTrt.txt", fileNameET="texitRateTrt.txt",
								fileNameES="texitRateScr.txt", fileNameA="talpha.txt",
								fileNameC="tcovps.txt") #parameters used by transmission model
  ### run the transmission model ###
